...
var _ = Describe("Alias address binding", func() {
	...
	Context("as the system-configured nameserver", func() {
		It("resolves the bosh-dns healthcheck", func() {
			cmd := exec.Command(boshBinaryPath, []string{"ssh", firstInstanceSlug, "-c", "dig -t A healthcheck.bosh-dns."}...)
			session, err := gexec.Start(cmd, GinkgoWriter, GinkgoWriter)
			Expect(err).NotTo(HaveOccurred())
		...
		Context("external processes changing /etc/resolv.conf", func() {
			BeforeEach(func() {
				backup := exec.Command(boshBinaryPath, []string{"ssh", firstInstanceSlug, "-c", "sudo cp /etc/resolv.conf /tmp/resolv.conf.backup"}...)
			...
			AfterEach(func() {
				restore := exec.Command(boshBinaryPath, []string{"ssh", firstInstanceSlug, "-c", "sudo mv /tmp/resolv.conf.backup /etc/resolv.conf"}...)
			...
			It("rewrites the nameserver configuration back to our dns server", func() {
				junkResolvConf := exec.Command(boshBinaryPath, []string{"ssh", firstInstanceSlug, "-c", "echo 'nameserver 192.0.2.100' | sudo tee /etc/resolv.conf > /dev/null"}...)
				session, err := gexec.Start(junkResolvConf, GinkgoWriter, GinkgoWriter)
				Expect(err).NotTo(HaveOccurred())
				Eventually(session, 10*time.Second).Should(gexec.Exit(0))

				Eventually(func() *gexec.Session {
					cmd := exec.Command(boshBinaryPath, []string{"ssh", firstInstanceSlug, "-c", "dig +time=3 +tries=1 -t A healthcheck.bosh-dns."}...)
					session, err = gexec.Start(cmd, GinkgoWriter, GinkgoWriter)
					Expect(err).NotTo(HaveOccurred())
					Eventually(session, 10*time.Second).Should(gexec.Exit())

					return session
				}, 15*time.Second, time.Second*2).Should(gexec.Exit(0))

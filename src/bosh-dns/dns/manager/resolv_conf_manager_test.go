...
var _ = Describe("ResolvConfManager", func() {
	...
	Describe("Read", func() {
		Context("When resolv.conf is empty", func() {
			BeforeEach(func() {
				fs.WriteFile("/etc/resolv.conf", []byte("")) //nolint:errcheck
			})
		...
		Context("When resolv.conf has multiple nameservers", func() {
			BeforeEach(func() {
				fs.WriteFile("/etc/resolv.conf", //nolint:errcheck
					[]byte(`
...
`))
	...
	Describe("SetPrimary", func() {
		...
		It("skips if resolvconf already has our server", func() {
			fs.WriteFileString("/etc/resolv.conf", `nameserver 192.0.2.100`) //nolint:errcheck
...
`))
		...
		It("prepends /etc/resolvconf/resolv.conf.d/head with our DNS server", func() {
			fs.WriteFileString("/etc/resolvconf/resolv.conf.d/head", //nolint:errcheck
				`# some comment
